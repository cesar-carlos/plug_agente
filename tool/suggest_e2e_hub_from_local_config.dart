...
void main(List<String> args) {
  ...
  if (config.hasAuthTokenInDb || config.hasStoredCredentials) {
    print('  E2E_HUB_TOKEN=<dart run tool/fetch_e2e_hub_token_from_local_config.dart --apply-token>');
  } else {
  ...
  if (!applyUrl || !applyAgentId) {
    print(
      'Tip:   dart run tool/suggest_e2e_hub_from_local_config.dart --apply-url --apply-agent-id',
    );
  }
}
