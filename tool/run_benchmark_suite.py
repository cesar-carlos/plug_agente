...
Migrate legacy logs with: python tool/migrate_benchmark_logs.py
Promote a good run to baseline: python tool/promote_benchmark_baseline.py
Compare runs over time: python tool/benchmark_evolution_report.py --include-history

dart run vs flutter test (FFI / isolates)
-----------------------------------------
transport_pipeline_json runs `dart run tool/benchmark_transport_pipeline.dart --path async`.
...
Prefer the flutter_test suite for regression gates and baselines:
  ...
  python tool/run_benchmark_suite.py --only transport_pipeline
...
TRANSPORT_DART_TOOL = "tool/benchmark_transport_pipeline.dart"
...
def render_report(summary: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "```bash",
            "python tool/compare_benchmark_summary.py \\",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.compare_baseline:
        from tool.compare_benchmark_summary import main as compare_main
