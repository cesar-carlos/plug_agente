...
Visao geral por tema (links externos para subdocs e ancoras internas):

- [Escopo atual](#escopo-atual-implementado) e
  [Status de implementacao](#status-de-implementacao)
- [Plug JSON-RPC Profile](#plug-json-rpc-profile)
...
- [Contrato JSON-RPC 2.0 (v2)](#contrato-json-rpc-20-v2)
- Metodos SQL: [`sql.cancel`](#metodo-sqlcancel-via-feature-flag),
  [`sql.executeBatch`](#batch-implementado),
  [`sql.bulkInsert`](#bulk-insert-nativo-implementado)
- [Catalogo de Erros](#catalogo-de-erros) e
  [Erros acionaveis](#erros-acionaveis-para-clientes)
- Metodos de agente: [`agent.getProfile`](#metodo-agentgetprofile),
  [`agent.getHealth`](#metodo-agentgethealth)
- **Metodos `agent.action.*` -> [socket_agent_actions.md](socket_agent_actions.md)**
  (run, validateRun, getExecution, cancel, policy metadata, auditoria remota)
- [`client_token.getPolicy`](#metodo-client_tokengetpolicy),
  [`rpc.discover`](#metodo-rpcdiscover)
- [Contrato obrigatorio de erro](#contrato-obrigatorio-de-erro-errordata),
  [Catalogo de category/reason](#catalogo-oficial-de-category-e-reason),
  [Politica de erro](#politica-de-erro-para-ui-e-logs)
- [Capabilities](#capabilities-negociacao-atual),
  [Compatibilidade e Fallback](#compatibilidade-e-fallback)
- [Client Token Authorization](#client-token-authorization-implementado),
  [Observabilidade de autorizacao](#observabilidade-de-autorizacao-implementado)
- [Politica de versao e deprecacao](#politica-de-versao-e-deprecacao),
  [Limites negociados por transporte](#limites-negociados-por-transporte)
- [Assinatura opcional de transporte/payload](#assinatura-opcional-de-transportepayload),
...
- Cliente binario obrigatorio: [socketio_client_binary_transport.md](socketio_client_binary_transport.md)
- Backlog ativo de evolucao: [socket_communication_backlog.md](socket_communication_backlog.md)
...
A descricao detalhada dos metodos `agent.action.run`, `validateRun`,
`getExecution` e `cancel`, alem dos metadados de policy de client token e da
auditoria remota append-only, foi extraida para
[socket_agent_actions.md](socket_agent_actions.md). O conteudo e parte do
contrato canonico publicado.
...
- `docs/communication/openrpc.json` — descoberta dos metodos via
  `rpc.discover`. Alteracoes neste arquivo sao validadas por
