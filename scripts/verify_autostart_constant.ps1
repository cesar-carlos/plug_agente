...
$canonical = (Get-Content "$rootDir\constants\autostart_arg.txt" -Raw).Trim()
if (-not $canonical) {
...
$appStrings = Get-Content "$rootDir\lib\core\constants\app_strings.dart" -Raw
...
$header = Get-Content "$rootDir\windows\runner\launch_args_constants.h" -Raw
...
$constantsIss = Get-Content "$rootDir\installer\constants.iss" -Raw
