...
int _parseWasteBytes() {
  final raw = E2EEnv.get('IDEMPOTENCY_FINGERPRINT_BENCHMARK_WASTE_BYTES')?.trim();
...
int _parseIterations() {
  final raw = E2EEnv.get('IDEMPOTENCY_FINGERPRINT_BENCHMARK_ITERATIONS')?.trim();
  final n = int.tryParse(raw ?? '');
  if (n == null || n < 3) {
    return 12;
  }
...
void main() {
  ...
  group('Idempotency fingerprint benchmark', () {
    test('should record resolveIdempotencyFingerprint latency for large params', () async {
      ...
      if (E2EEnv.get('IDEMPOTENCY_FINGERPRINT_BENCHMARK_RECORD') == 'true') {
        final custom = E2EEnv.get('IDEMPOTENCY_FINGERPRINT_BENCHMARK_FILE')?.trim();
