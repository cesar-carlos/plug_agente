...
class JarActionProcessRunner implements AgentActionLocalRunner {
  ...
  Future<Result<AgentActionProcessResult>> run({
    ...
    if (definition.config is! JarActionConfig) {
      return Failure(
        ActionValidationFailure.withContext(
          message: 'Jar process runner received an invalid action config.',
          context: {
    ...
    if (adapter is! JarActionAdapter) {
      return Failure(
        ActionValidationFailure.withContext(
          message: 'Jar process runner received an invalid action adapter.',
          context: {
            'action_id': definition.id,
            'action_type': definition.type.name,
            'phase': AgentActionProcessConstants.executionPreflightPhase,
    ...
    return _lifecycle.execute(
      ...
      failureMessages: const AgentActionProcessFailureMessages(
        processStartMessage: 'Failed to start jar action process.',
        processRuntimeMessage: 'Jar action process failed before completion.',
        ...
        processExceptionCode: AgentActionFailureCode.javaNotFound,
      ),
    );
  }
