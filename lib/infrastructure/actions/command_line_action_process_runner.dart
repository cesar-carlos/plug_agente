...
class CommandLineActionProcessRunner implements AgentActionLocalRunner {
  ...
  Future<Result<AgentActionProcessResult>> run({
    ...
    if (definition.config is! CommandLineActionConfig) {
      return Failure(
    ...
    if (adapter is! CommandLineActionAdapter) {
      return Failure(
        ActionValidationFailure.withContext(
          message: 'Command line process runner received an invalid action adapter.',
          context: {
            'action_id': definition.id,
            'action_type': definition.type.name,
            'phase': AgentActionProcessConstants.executionPreflightPhase,
