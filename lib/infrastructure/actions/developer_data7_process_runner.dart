...
class DeveloperData7ProcessRunner implements AgentActionLocalRunner {
  ...
  Future<Result<AgentActionProcessResult>> run({
    ...
    if (definition.config is! DeveloperActionConfig) {
      return Failure(
        ActionValidationFailure.withContext(
          message: 'Developer Data7 process runner received an invalid action config.',
          context: {
    ...
    if (adapter is! DeveloperData7ActionAdapter) {
      return Failure(
        ActionValidationFailure.withContext(
          message: 'Developer Data7 process runner received an invalid action adapter.',
          context: {
            'action_id': definition.id,
            'action_type': definition.type.name,
            'phase': AgentActionProcessConstants.executionPreflightPhase,
    ...
    return _lifecycle.execute(
      ...
      failureMessages: const AgentActionProcessFailureMessages(
        processStartMessage: 'Failed to start Developer Data7 process.',
        processRuntimeMessage: 'Developer Data7 process failed before completion.',
        processStartUserMessage:
            'Nao foi possivel iniciar o Executor.exe. Verifique o caminho do executavel e do projeto.',
        processRuntimeUserMessage:
            'Falha ao executar o projeto Data7. Revise a configuracao da acao e tente novamente.',
      ),
    );
  }
