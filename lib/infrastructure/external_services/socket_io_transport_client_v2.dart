...
class SocketIOTransportClientV2 implements ITransportClient {
  ...
  ProtocolCapabilities _localCapabilities() {
    ...
    if (_cachedLocalCapabilities != null && _localCapabilitiesCacheKey == cacheKey) {
  ...
  TransportPipeline _createSendPipeline({
    ...
    final effectiveMode = clientOutboundMode ?? _featureFlags.outboundCompressionMode;
    final String pipelineCompression;
    if (effectiveMode == OutboundCompressionMode.none || negotiatedCmp == 'none') {
    ...
    try {
      ...
      if (_featureFlags.enablePerRequestOutboundCompression) {
        ...
        if (batchOutboundResult.isError()) {
          final failure = batchOutboundResult.exceptionOrNull() as domain.Failure?;
          if (failure != null) {
            await _sendSchemaValidationError(
              null,
              _validationFailureCode(failure),
  ...
  Future<dynamic> _prepareOutgoingPayloadAsync(
    ...
    } else if (logicalPayload is List<dynamic> && _pendingRpcResponseIsBatchArray) {
      _pendingRpcResponseIsBatchArray = false;
      _pendingBatchOutboundExplicitMode = null;
    }
    ...
    if (_featureFlags.enableSocketOutboundCompressionDebugLog &&
        (event == 'rpc:response' || event == 'rpc:chunk' || event == 'rpc:complete')) {
      final negotiated = _hasReceivedCapabilities ? _currentProtocol.compression : 'gzip';
      final gzipCappedByNegotiation =
          clientOutboundMode == OutboundCompressionMode.gzip && pipeline.compression == 'none' && negotiated == 'none';
