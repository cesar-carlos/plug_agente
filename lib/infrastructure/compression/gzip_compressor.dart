...
class GzipCompressor {
  Future<Result<List<Map<String, dynamic>>>> compress(List<Map<String, dynamic>> data) async {
    try {
      ...
      if (compressedBytes == null) {
        return Failure(domain.CompressionFailure('Failed to compress data: encoder returned null'));
      }
      ...
      return Success([
        {'compressed_data': base64String, 'is_compressed': true, 'original_size': bytes.length},
      ]);
    } catch (e) {
  ...
  Future<Result<List<Map<String, dynamic>>>> decompress(List<Map<String, dynamic>> data) async {
    try {
      ...
      if (data.length == 1 && data.first.containsKey('compressed_data') && data.first['is_compressed'] == true) {
        ...
        return Success(List<Map<String, dynamic>>.from(jsonDecode(jsonString)));
      }

      // Return data as-is if not compressed
      return Success(data);
    } catch (e) {
      return Failure(domain.CompressionFailure('Failed to decompress data: $e'));
    }
  }
}
