...
class SyncAgentProfileWithHub {
  ...
  Future<AgentProfileHubSyncResult> call({
    ...
    if (accessToken.trim().isEmpty || serverUrl.trim().isEmpty || agentId.trim().isEmpty) {
  ...
  Failure _asFailure(Object error) {
    ...
    return ServerFailure.withContext(
      message: 'Failed to sync agent profile with hub',
      cause: error,
      context: const {'operation': 'syncAgentProfileWithHub'},
    );
  }
}
